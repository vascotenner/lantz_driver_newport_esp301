...
class SMC100(MotionController):
    ...
    DEFAULTS = {
                ...
                'ASRL': {
                    ...
                    'flow_control': constants.VI_ASRL_FLOW_XON_XOFF,
                    },
                }

    def __init__(self, *args, **kwargs):
        self.motionaxis_class = kwargs.pop('motionaxis_class',
                                           MotionAxisSMC100)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--test', type=bool, default=False,
                        help='Move stage 100 times and read position')
    ...
    with sm(args.port, timeout=100) as inst:
        # Find the status of all axes:
        for axis in inst.axes:
            msg = ['{}: {}'.format(name, val) for name, val in
                   axis.refresh().items()]
            print('Axis {} properties:'.format(axis.num))
            print('    ' + '\n    '.join(msg))
        if args.test:
            print('### Reading stability test ###')
            for i in range(100):
                print(i, inst.axes[0].position)
                inst.axes[0].position += 0.5 * ureg.um
                print(i, inst.axes[0].position)
                inst.axes[0].position -= 0.5 * ureg.um
